...
text는 다르게 표현해도 되지만 quote는 원문의 영문 대소문자, 단어 사이 띄어쓰기,
표기를 글자 그대로 유지하세요. 예를 들어 원문이 "API 서버를 개발했습니다."면
text는 "api 서버 개발 경험을 설명했다."로 써도 되지만 quote는
"API 서버를 개발했습니다."여야 합니다. "api 서버를"이나 "API서버를"처럼 바꾼
quote는 원문에서 찾지 못해 그 항목 전체가 결과에서 빠집니다.
...
class OpenAISummarizer:
    ...
    async def summarize(self, transcript: TranscriptSnapshot) -> SummaryResult:
        ...
        if not any(u.speaker is SpeakerRole.CANDIDATE for u in utterances):
