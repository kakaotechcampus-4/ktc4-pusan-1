...
| --- | --- | --- | --- |
| TC-IN-01 | 정상 가상 TranscriptSnapshot | 세션과 발화 revision 5개를 읽는다 | `test_sample_snapshot_loads` |
| TC-IN-02 | `passType: INTERIM` 중간 발화 포함 | 중간 발화를 Q&A·GPT 입력·근거에서 제외한다 | `test_final_utterances_exclude_provisional_results`, `test_real_sdk_request_and_parsing_with_stubbed_http` |
| TC-IN-03 | 입력 발화 순서가 뒤섞임 | `seq` 순으로 처리한다 | `test_final_utterances_are_sorted_by_seq` |
...
| TC-IN-05 | 동일 ID에서 트랙·화자·seq·시작 시각 변경 | 입력 검증 실패 | `test_conflicting_identity_in_revisions_is_rejected` |
| TC-IN-06 | 음수 시작 시각 또는 종료 < 시작 | 입력 검증 실패 | `test_utterance_rejects_negative_start`, `test_utterance_rejects_reversed_time_range` |
| TC-IN-07 | FINAL 발화가 빈 문자열 또는 공백뿐 | 입력 검증 실패 | `test_blank_final_content_is_rejected` 2건 |
...
| TC-QA-02 | 같은 화자의 연속 발화 조각 | 같은 질문 또는 답변 안에서 순서대로 합친다 | `test_orphans_and_unanswered_question_are_preserved` |
| TC-QA-03 | 첫 질문 전에 지원자 발화 | 버리지 않고 `unpairedUtteranceIds`에 남긴다 | `test_orphans_and_unanswered_question_are_preserved` |
| TC-QA-04 | 질문 뒤 final 답변 없음 | 빈 답변의 Q&A와 `UNANSWERED_QUESTIONS` 경고를 남긴다 | `test_unanswered_question_survives_without_llm` |
| TC-QA-05 | 두 세션을 동시에 분석 | 세션별 결과와 근거가 섞이지 않고 입력 변화가 결과에 반영된다 | `test_concurrent_sessions_stay_separate` |
| TC-QA-06 | 이전 세션의 LLM 실패 후 다음 세션 분석 | 다음 세션은 독립적으로 정상 완료된다 | `test_provider_failure_preserves_qa_and_recovers_next_session` |
...
| TC-GR-02 | 면접관 발화를 지원자 근거로 인용 | 해당 요약 항목 전체를 제외한다 | `test_invalid_claim_is_removed_from_all_display_fields[bad_point1]` |
| TC-GR-03 | 원문에 없는 인용 또는 빈 인용 | 해당 요약 항목 전체를 제외한다 | `test_invalid_claim_is_removed_from_all_display_fields[bad_point2]`, `[bad_point3]` |
| TC-GR-04 | 요약 수치가 해당 인용문에 없음 | 해당 요약 항목 전체를 제외한다 | `test_invalid_claim_is_removed_from_all_display_fields[bad_point4]` |
| TC-GR-05 | 인용이 없거나 요약문이 공백 | 해당 요약 항목 전체를 제외한다 | `test_invalid_claim_is_removed_from_all_display_fields[bad_point5]`, `[bad_point6]` |
...
| TC-SEC-02 | 잘못된 입력에 비공개 문자열 포함 | CLI 오류 출력에 입력 본문을 노출하지 않는다 | `test_analyze_invalid_input_does_not_echo_sensitive_data` |
| TC-SEC-03 | API Key 미설정 | 가짜 성공으로 대체하지 않고 `OPENAI_API_KEY_MISSING`, 종료 코드 2를 반환한다 | `test_analyze_missing_key_does_not_fall_back_to_fake_success` |
...
| TC-EV-01 | 핵심 사실 또는 원문에 없는 수치 검사 | 공백·영문 대소문자를 정규화하고 없는 수치를 중복 없이 찾는다. 이 완화된 정규화는 수치·핵심 사실 점검에만 쓰고 인용 일치 판정에는 쓰지 않는다 | `test_evaluation.py` 6건 |
...
2026-09-11 로컬 결과 (인용 검증 회귀 테스트 추가 후):
...
- 근거 ID는 `u-002` 또는 `u-004`이며 `u-004`의 INTERIM 본문은 사용하지 않는다.
...
| 실행 시각 | 모델 | 상태 | elapsedMs | 요약 항목 | 제외 항목 | 핵심 사실 누락 | 비고 |
| --- | --- | --- | ---: | ---: | ---: | --- | --- |
...
- 인용 검증에서 제외된 항목이 있으면 `partial`과 `rejectedPointCount`로 드러난다.
