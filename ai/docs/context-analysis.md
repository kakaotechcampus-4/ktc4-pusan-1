...
백엔드가 전달한 Transcript 스냅샷
  → 입력 검증·발화 ID별 최종 수정본 선택·시각 순 정렬
...
최상위 객체는 `Transcript`이며 별도의 HTTP 경로나 응답 봉투를 가정하지 않는다.

| 필드 | 의미 |
| --- | --- |
| `session_id` | 빈 문자열이 아닌 세션 식별자 |
| `stage` | `live`(기본값) 또는 `realigned`; 누락 시 보수적으로 잠정본으로 표시 |
| `utterances` | 발화 배열, 빈 배열 허용 |
| `utterance_id` | 발화 식별자; 동일 발화의 수정본은 동일 ID |
| `speaker` | `interviewer` 또는 `candidate`; BE/STT가 트랙으로 결정 |
| `text` | **전체 발화 문자열**; 토큰 델타 아님. 최종 발화는 공백만 허용하지 않음 |
| `started_at_ms`, `ended_at_ms` | 세션 시작 기준 상대 밀리초; 0 이상, 끝 ≥ 시작 |
| `is_final` | 발화 단위 STT 확정 여부(기본 `true`); 배치 전사 확정과 다른 개념 |

동일 ID가 여러 번 있으면 배열에서 **마지막 `is_final: true` 전체 문자열**을 쓴다.
그 뒤에 중간본이 도착해도 최종본을 덮지 않는다. 같은 ID의 화자·시작 시각은
변경할 수 없다. 본문·종료 시각은 정정할 수 있다. BE가 수정 순서를 보장해야 하며,
순서가 없는 이벤트를 안전하게 병합하는 규칙은 이번 범위에 없다.

라이브 STT의 `is_final: true`는 발화가 끝났다는 뜻이다. `stage: live`인 이상
...
| 필드 | 의미 |
| --- | --- |
| `session_id`, `transcript_stage` | 입력 식별자와 전사 단계 |
| `status` | `completed`, `partial`, `failed`, `empty` |
| `qa_pairs` | 질문·답변 원문, 각각의 발화 ID 배열, 시작/종료 밀리초 |
| `unpaired_utterance_ids` | 앞선 면접관 질문 없이 등장한 지원자 발화 ID; 누락하지 않고 별도 보존 |
| `summary_result` | 성공 시 요약 객체; 실패·분석할 지원자 발화 없음이면 `null` |
| `summary_result.points` | `text`와 `evidence[]`로 구성된 요약 항목 |
| `evidence` | 원문 `quote`, `utterance_id`, `speaker`, 발화 구간의 시작/종료 밀리초 |
| `summary_result.summary`, `key_points` | 검증을 통과한 `points`에서 파생한 표시용 텍스트 |
| `source_utterance_ids` | 실제 인용한 발화만 중복 제거; 전사 전체 ID를 근거처럼 넣지 않음 |
| `model` | SDK 응답의 실제 모델 ID, 오프라인은 `extractive-baseline` |
| `rejected_point_count` | 검증에서 제외한 요약 항목 수 |
| `warnings`, `error`, `elapsed_ms` | 경고, 안전한 오류 코드·재시도 가능 여부, 전체 분석 소요 시간 |
...
- `quote`가 해당 원문에 정확한 부분 문자열로 존재하지 않음.
...
- Q&A는 화자 전환 기준이다. 면접관의 맞장구·끼어들기와 실제 새 질문은 구별하지 않는다.
