...
├── hooks/
│   └── useInterviewRoom.ts   LiveKit 연결, 트랙 구독, 전사 WebSocket
├── lib/                      공용 유틸
├── mocks/                    ⚠️ 임시 — 아래 참고
├── pages/
...
라우팅, 방 코드 입력 화면, 카메라·마이크 권한 UI, 로컬 프리뷰, 개별 예외 화면(없는 코드 / 정원 초과 / 권한 거부), 인증.

[#5](https://github.com/kakaotechcampus-4/ktc4-pusan-1/issues/5) · [#8](https://github.com/kakaotechcampus-4/ktc4-pusan-1/issues/8) 범위입니다.

## `src/mocks/` 는 임시입니다

BE·AI 없이 화면을 확인하기 위한 코드입니다. LiveKit 도 API 도 붙이지 않고 zustand 스토어에 가짜 이벤트만 흘립니다.
