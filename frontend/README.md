...
| 면접 요약        | 엔드포인트가 명세에 없어 FE 가 형태를 제안한 상태                                                |

### 의도적으로 넣지 않은 것

- **인증·로그인** — 프로토타입 범위 밖. `role` 을 클라이언트가 선언하는 현재 구조는
  인증이 붙으면 서버 판단으로 바뀝니다
- **AI 추천 질문** — 담당 이슈가 없어 프로토타입에서 제외했습니다 (2026-09-09). 구현되면 다시 넣습니다
- **면접 기록(S3) · 기업 컨텍스트(S1) · 지원자 목록(S4)**

### 코드는 있으나 아직 마운트되지 않는 것

아래 둘은 **BE 가 붙었을 때 쓸 실제 구현**입니다. 명세에 맞춰져 있지만 서버가 없어
프로토타입에서는 대신 `mocks/InterviewRoomPreview` 를 그립니다. 지우지 않고 남겨 둡니다.

| 파일                        | 하는 일                                            |
| --------------------------- | -------------------------------------------------- |
| `hooks/useInterviewRoom.ts` | `join` → LiveKit 접속 → 트랙 발행 → 원격 트랙 구독 |
| `pages/InterviewRoom.tsx`   | 위 훅과 `InterviewRoomView` 를 잇는 연결부         |

`api/interview.ts` 의 `getInterview` · `getSessionState` 도 같은 이유로 정의만 되어 있습니다.

### 확정되지 않아 기본값으로 진행한 것

BE 명세에 없거나 미정이라 FE 가 기본값을 정한 항목입니다. 확정되면 "고칠 곳" 한 군데만 바꾸면 됩니다.

| 항목             | FE 기본값                            | 고칠 곳                                 |
| ---------------- | ------------------------------------ | --------------------------------------- |
| 에러 응답 본문   | HTTP status 로만 분기                | `api/interview.ts` 의 `FAILURE_BY_CODE` |
| 정원 초과 구분   | 409 를 "이미 종료" 와 합쳐 한 문구   | 위와 같음                               |
| 토큰 TTL         | 재발급 없음. 만료 시 새로고침        | `useInterviewRoom` 의 `Disconnected`    |
| 요약 엔드포인트  | `GET /sessions/{id}/summary` 로 가정 | `api/interview.ts` 의 `getSummary`      |
| 지원자 이름      | 표시하지 않음                        | `InterviewRoomView`                     |
| `role` 선언 주체 | 클라이언트가 요청에 실어 보냄        | `joinSession` 호출부                    |
