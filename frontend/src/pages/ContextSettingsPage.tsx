...
import { createTextDoc, deleteDoc, getContext, uploadDoc } from '../api/context';
...
import { TextPaste } from '../components/context/TextPaste';
import { prepareDocs, type PreparedDoc } from '../lib/prepareDocs';
...
interface PendingDoc extends ContextDoc {
  /** 서버가 아직 모르는 상태 — 올리는 중이거나, 올리다 실패했다 */
...
export default function ContextSettingsPage() {
  ...
  /**
   * 올리는 중이거나 실패한 파일을 들고 있는다.
   *
   * 실패했을 때 사용자가 파일을 처음부터 다시 고르지 않아도 되게 하려면, 고른 File 객체를
   * 어딘가 들고 있어야 한다. 여기가 그 자리다.
   */
  const [attempts, setAttempts] = useState<
    Record<string, { prepared: PreparedDoc; category: DocCategory }>
  >({});

  /**
   * 이미 올라간 문서의 내용 지문.
   *
   * attempts 는 업로드가 끝나면 비워지므로, 중복 판별용 지문은 따로 남겨야 한다.
   * 안 그러면 방금 올린 문서를 이름만 바꿔 다시 올려도 걸러지지 않는다.
   */
  ...
  const upload = useMutation({
    ...
    onSuccess: (doc, { tempId, category }) => {
      ...
      const print = attempts[tempId]?.prepared.fingerprint;
      if (print) setFingerprints((prev) => ({ ...prev, [doc.id]: print }));
      setAttempts((prev) => {
        const next = { ...prev };
        delete next[tempId];
        return next;
    ...
    // 실패한 항목은 목록에서 지우지 않는다. 그 자리에 "다시 시도" 를 띄우려면 남아 있어야 한다.
    onError: (_e, { tempId }) => {
      setPending((prev) =>
        prev.map((d) => (d.id === tempId ? { ...d, status: 'failed', progress: undefined } : d)),
      );
      setNotice('올리지 못했습니다. 목록에서 다시 시도할 수 있습니다.');
    },
  });

  /** 준비가 끝난 파일 하나를 올린다. 처음 시도와 재시도가 같은 길을 쓴다. */
  const startUpload = (prepared: PreparedDoc, category: DocCategory, id?: string) => {
    const tempId = id ?? `tmp_${Date.now()}_${prepared.file.name}`;
    setAttempts((prev) => ({ ...prev, [tempId]: { prepared, category } }));
    ...
    setPending((prev) => {
      ...
      return prev.some((d) => d.id === tempId)
        ? prev.map((d) => (d.id === tempId ? entry : d))
        : [...prev, entry];
    });
    upload.mutate({ file: prepared.file, tempId, category });
  };

  /** 붙여넣은 텍스트는 파싱이 없어 한 번의 요청으로 끝난다. */
  const addText = useMutation({
    mutationFn: ({ title, body }: { title: string; body: string; category: DocCategory }) =>
      createTextDoc(CONTEXT_ID, title, body),
    onSuccess: (doc, { category }) => {
      setCategories((prev) => ({ ...prev, [doc.id]: category }));
      void qc.invalidateQueries({ queryKey });
    },
    onError: () => setNotice('텍스트를 저장하지 못했습니다.'),
  });
  ...
  const remove = useMutation({
    ...
    onSettled: (_data, _error, docId) => {
      // 지운 문서의 지문은 잊는다. 지웠으면 같은 문서를 다시 올릴 수 있어야 한다.
  ...
  const handleFiles = async (files: File[], category: DocCategory) => {
    setNotice(null);
    // 형식·내용·중복·스캔본을 한 번에 본다. 여기서 걸리면 서버까지 가지 않는다.
    const { accepted, rejected } = await prepareDocs(files, [
      ...Object.values(fingerprints),
      ...Object.values(attempts)
        .map((a) => a.prepared.fingerprint)
        .filter((f): f is string => Boolean(f)),
    ]);
    if (rejected.length) {
      setNotice(rejected.map((r) => `${r.name} — ${r.message}`).join(' / '));
    }
    for (const prepared of accepted) startUpload(prepared, category);
  };

  /** 실패한 항목을 같은 파일로 다시 올린다. */
  const retryUpload = (tempId: string) => {
    const attempt = attempts[tempId];
    if (!attempt) return;
    setNotice(null);
    startUpload(attempt.prepared, attempt.category, tempId);
  };

  // 검사에서 얻은 안내를 카드에 붙인다 (스캔본 경고, 쪽수).
  const notes = Object.fromEntries(
    Object.entries(attempts).map(([id, a]) => [
      id,
      { warning: a.prepared.warning, hint: a.prepared.hint },
    ]),
  );
          ...
          <>
            ...
            >
              ...
              <DropZone
                ...
                onFiles={(files) => void handleFiles(files, 'jd')}
                disabled={uploading}
              />

              {/* 채용 공고는 대개 복사할 수 있다. 텍스트로 넣으면 파일보다 빠르고 정확하다. */}
              <details className="border-border-base rounded-xl border p-4">
                <summary className="text-ink-muted cursor-pointer text-[14px]">
                  파일 대신 텍스트로 붙여넣기
                </summary>
                <div className="mt-3">
                  <TextPaste
                    ...
                    onSubmit={(title, body) => addText.mutate({ title, body, category: 'jd' })}
                  />
                </div>
              </details>
            </Section>
            ...
            >
              ...
              <DropZone
                ...
                onFiles={(files) => void handleFiles(files, 'internal')}
                disabled={uploading}
              />
              ...
              <details className="border-border-base rounded-xl border p-4">
                ...
                <div className="mt-3">
                  <TextPaste
                    disabled={addText.isPending}
                    placeholder="컬처 코드나 핵심가치 내용을 붙여넣으세요."
                    onSubmit={(title, body) =>
                      addText.mutate({ title, body, category: 'internal' })
                    }
                  />
                </div>
              </details>
...
function DocList({
  ...
  /** 문서별 안내 — 스캔본 경고, 쪽수 */
  notes?: Record<string, { warning?: string; hint?: string }>;
}) {
  if (docs.length === 0) {
    ...
    <div className="flex flex-col gap-2.5">
      {docs.map((doc) => (
        <DocCard
          key={doc.id}
          doc={doc}
          // 업로드 중이거나 실패한 문서는 서버가 모르므로 지울 수 없다.
          onDelete={
            doc.status === 'uploading' || doc.status === 'failed'
              ? undefined
              : () => onDelete(doc.id)
          }
          onRetry={onRetry && doc.status === 'failed' ? () => onRetry(doc.id) : undefined}
          warning={notes?.[doc.id]?.warning}
          hint={notes?.[doc.id]?.hint}
        />
      ))}
    </div>
  );
