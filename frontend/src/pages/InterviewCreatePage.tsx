...
export default function InterviewCreatePage() {
  ...
  // 검사에서 얻은 안내(스캔본 경고·쪽수). 카드에 붙여 보여준다.
  const [prepared, setPrepared] = useState<PreparedDoc | null>(null);
  /** 이력서 업로드만 실패했을 때, 다시 붙일 대상 면접 */
    ...
    mutationFn: async () => {
      ...
      if (resume) {
        ...
        try {
          ...
          // 다만 어느 면접에 붙일 이력서였는지는 기억해 둔다 — 다시 시도하려면 필요하다.
          setDoc((prev) => prev && { ...prev, status: 'failed', progress: undefined });
          setLastInterviewId(interview.interviewId);
          setNotice(
            '이력서를 올리지 못했습니다. 면접은 만들었고, 아래에서 다시 시도할 수 있습니다.',
          );
        }
      }
  ...
  const handleFiles = async (files: File[]) => {
    ...
    // 형식·내용·스캔본을 미리 본다. 중복은 한 장짜리라 볼 것이 없다.
    const { accepted, rejected } = await prepareDocs([file], []);
    const first = accepted[0];
    if (!first) {
      setNotice(rejected[0]?.message ?? '이 파일은 올릴 수 없습니다.');
      return;
    }

    setPrepared(first);
    setResume(first.file);
    setDoc({
      id: `resume_local_${Date.now()}`,
      name: first.file.name,
      kind: first.kind,
      sizeBytes: first.file.size,
   ...
   * 이력서만 실패했을 때 다시 올린다.
   *
   * 면접은 이미 만들어졌으므로 다시 만들지 않는다 — 같은 면접에 이력서만 붙인다.
   */
  const retryResume = async () => {
    if (!resume || !lastInterviewId) return;
    setNotice(null);
    setDoc((prev) => prev && { ...prev, status: 'uploading', progress: 0 });
    try {
      await uploadResume(lastInterviewId, resume, (ratio) =>
        setDoc((prev) => prev && { ...prev, progress: ratio }),
      );
      setDoc((prev) => prev && { ...prev, status: 'ready', progress: undefined });
      setLastInterviewId(null);
      ...
      setDoc((prev) => prev && { ...prev, status: 'failed', progress: undefined });
      setNotice('이력서를 올리지 못했습니다. 잠시 후 다시 시도해주세요.');
    }
  };
            ...
            <SectionCard icon="upload_file" title="지원자 이력서" badge="PDF · DOCX · 50MB 이하">
              {doc ? (
                <DocCard
                  ...
                  onRetry={doc.status === 'failed' ? () => void retryResume() : undefined}
                />
              ) : (
                <DropZone
